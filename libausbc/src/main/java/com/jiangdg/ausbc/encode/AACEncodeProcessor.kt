...
// import com.jiangdg.natives.LameMp3 // Temporarily disabled due to libnative module issues
import java.io.File
    ...
    fun recordMp3Start(audioPath: String?, callBack: ICaptureCallBack) {
        mAudioThreadPool.submit {
            ...
            try {
                ...
                // LameMp3.lameInit(sampleRate, channelCount, sampleRate, BIT_RATE, DEGREE_RECORD_MP3) // Temporarily disabled
                mMainHandler.post {
                    callBack.onBegin()
                }
                ...
                while (mRecordMp3State.get()) {
                    mRecordMp3Queue.poll()?.apply {
                        ...
                        // val encodeSize = LameMp3.lameEncode(tmpData, null, tmpData.size, mp3Buf) // Temporarily disabled
                ...
                // val flushSize = LameMp3.lameFlush(mp3Buf) // Temporarily disabled
                // if (flushSize > 0) {
                ...
                try {
                    fos?.close()
                    fos = null
                    // LameMp3.lameClose() // Temporarily disabled
                    releaseAudioRecord()
                    mMainHandler.post {
